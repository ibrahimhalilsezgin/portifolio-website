  ...
  import { PUBLIC_BACKEND_URL } from "$env/static/public";
    ...
    async function submit() {
        const response = await axios({
            method:'post',
            url:PUBLIC_BACKEND_URL + '/certificates/createcertificate',
            headers:{
        ...
        if(response.status == 200) {
            ...
            imageFormData.append('certificatesImage', file[0]);
            imageFormData.append('title', formData.title)
            await axios({
                method:'post',
                url:PUBLIC_BACKEND_URL + '/certificates/upload',
                headers:{
                    Authorization: 'Bearer ' + getCookie('token')
                },
                data: imageFormData
...
{#if success}
    <div class="bg-green-500 p-2 rounded-full flex jusfiy-center text-center items-center text-black" transition:slide>
        <div class="w-full text-xl">
            Certification is Sucessfully Created.
        </div>
    </div>
{/if}
<div class="h-full flex justify-center items-center">
    <div class="flex flex-col h-4/5 w-3/5 rounded-4xl bg-adminbg-2 border-adminbg-3 border shadow-2xl shadow-black/10 items-center jusfiy-center">
        ...
        <button class="w-full mt-7 hover:text-yellow-400 cursor-pointer" on:click={() => goto('/admin/certificates')}>
            <ArrowLeftIcon size={60}/>
        </button>
        <h1 class="md:text-4xl pt-3">Create Certification</h1>
