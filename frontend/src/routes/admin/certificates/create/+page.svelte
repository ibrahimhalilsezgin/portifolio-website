  ...
  import { goto } from "$app/navigation";
    ...
    async function submit() {
        const response = await axios({
            method:'post',
            url:'http://localhost:3000/certificates/createcertificate',
            headers:{
        ...
        if(response.status == 200) {
            ...
            await axios({
                method:'post',
                url:'http://localhost:3000/certificates/upload',
                headers:{
                    Authorization: 'Bearer ' + getCookie('token')
                },
                data: imageFormData
...
<div class="h-full flex justify-center items-center">
    <div class="flex flex-col h-4/5 w-3/5 rounded-4xl bg-adminbg-2 border-adminbg-3 border shadow-2xl shadow-black/10 items-center jusfiy-center">
        ...
        <div class="w-full mt-7 hover:text-yellow-400 cursor-pointer" on:click={() => goto('/admin/certificates')}>
